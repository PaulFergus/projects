/*
 * Class Title:         VideoPipeMsgListener
 ...
 */
package NASUF.src.uk.ac.livjm.listener.pipe.impl.video;
...
public class VideoPipeMsgListener extends ServicePipeMsgListener{
    //The service logic
    ...
    //parameter, which is the port address the video 
    //device listens for data on.
    public VideoPipeMsgListener(String port) {
        this.port = port;
    ...
    //This getter method returns the state variable for the video
    //device.
    ...
    public void recieveMedia(
        ...
        try{
            ...
            if(NASUFLogger.isEnabledFor(Level.ERROR))
                NASUFLogger.error("VideoPipeMsgListener: recieveMedia: " + 
                    e.toString());
        }
	//If the reciever service has not been started then
	    ...
	    //Update the state variable to indicate that the 
		    ...
		    //the data has finished being transmitted.
                    Thread.sleep(1000);
		//Close the video reciever service and destroy the instance
                avReceive.close();
                avReceive = null;
    ...
    //video service. This event acts as the control entry point.
    public void pipeMsgEvent(net.jxta.pipe.PipeMsgEvent pipeMsgEvent) {
		...
		//First check o see if the audio device is listening for
            ...
            if(NASUFLogger.isEnabledFor(Level.ERROR))
                NASUFLogger.error("DeCapPipeMessageListener Error: " + toString());
        }
    }
