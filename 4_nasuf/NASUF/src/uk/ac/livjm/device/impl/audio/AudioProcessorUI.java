...
public class AudioProcessorUI extends javax.swing.JFrame {
    ...
    //Default constructor.
    public AudioProcessorUI() {
        initComponents();
        this.setSize(270, 130);
        //The default behaviour is to announce the device when 
        //the user interface is executed.
        announceDevice();
    }
    private void initComponents() {//GEN-BEGIN:initComponents
        ...
        lblStopped.setForeground(java.awt.Color.blue);
        lblStopped.setVerifyInputWhenFocusTarget(false);
    ...
    private void removeDevice(){
        try{
            ...
            if(NASUFLogger.isEnabledFor(Level.ERROR))
                NASUFLogger.error("AudioProcessorUI: removeDevice: " + 
                    e.toString());
        }
    ...
    private void btnAnnounceButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAnnounceButtonActionPerformed
        if (evt.getSource() == btnAnnounceButton) {
            ...
            if (currentLabel.equals(DeviceConstants.LABEL_ANNOUNCE)) {
                // announce the device in the UPnP network
    ...
    //When form is exited ensure that the device
